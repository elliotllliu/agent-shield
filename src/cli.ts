...
import { runEngines, getAllEngines, renderTerminalReport, renderJsonReport, renderMarkdownReport } from "./engines/index.js";
...
program
  ...
  .option("--engines <engines>", "Run multiple engines: agentshield,snyk,cisco,tencent,all (default: agentshield)")
  ...
  .action(async (directory: string, options: { json?: boolean; sarif?: boolean; html?: boolean; output?: string; failUnder?: number; disable?: string; enable?: string; ai?: boolean; provider?: string; model?: string; engines?: string }) => {
    ...
    // Multi-engine aggregated scan
    if (options.engines) {
      const engineNames = options.engines.split(",").map((s: string) => s.trim());
      console.error(`🔧 Running multi-engine scan: ${engineNames.join(", ")}...`);
      
      const aggregated = await runEngines(scanTarget, engineNames, {
        timeout: 60_000,
        includeLow: true,
      });
      aggregated.filesScanned = result.filesScanned;

      if (options.json) {
        const out = renderJsonReport(aggregated);
        if (options.output) { writeFileSync(options.output, out); console.error(`📄 Written to ${options.output}`); }
        else console.log(out);
      } else {
        const out = renderTerminalReport(aggregated);
        if (options.output) {
          const md = renderMarkdownReport(aggregated);
          writeFileSync(options.output, md);
          console.error(`📄 Markdown report written to ${options.output}`);
        }
        console.log(out);
      }

      if (options.failUnder !== undefined && aggregated.score < options.failUnder) {
...
// Setup command — check and install scan engines
program
  .command("setup")
  .description("Check availability of scan engines and show install instructions")
  .action(async () => {
    console.log("\n🛡️  AgentShield Engine Setup\n");

    const engines = getAllEngines();

    for (const engine of engines) {
      const available = await engine.available();
      const status = available ? "\x1b[32m✅ Available\x1b[0m" : "\x1b[33m⬜ Not installed\x1b[0m";
      console.log(`  ${status}  ${engine.displayName} (${engine.name})`);

      if (!available) {
        const instructions = engine.setup().split("\n");
    ...
    const availableCount = (await Promise.all(engines.map((e) => e.available()))).filter(Boolean).length;
    console.log(`  ${availableCount}/${engines.length} engines available.\n`);
  });
